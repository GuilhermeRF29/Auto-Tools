...
const VaultView = ({ currentUser }: { currentUser: any }) => {
  ...
  const handleAddCred = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: currentUser.id, servico: newCred.site, login: newCred.user, senha: newCred.pass })
      });
      setIsAdding(false);
  ...
  if (!isUnlocked) {
    return (
      <div className="h-full flex items-center justify-center animate-in fade-in zoom-in-95 duration-300">
        <Card className="w-full max-w-md p-8 text-center">
          <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <Lock size={32} className="text-slate-600" />
          </div>
          <h2 className="text-2xl font-bold text-slate-800 mb-2">Cofre Bloqueado</h2>
          <p className="text-slate-500 mb-8">Insira sua senha mestre para acessar as credenciais.</p>
          ...
          <form onSubmit={handleUnlock} className="space-y-4">
            <input 
              ...
              className="w-full border border-slate-300 rounded-md p-3 text-center text-lg focus:ring-2 focus:ring-blue-500 outline-none"
              autoFocus
            />
            <Button type="submit" className="w-full py-3 text-base">Desbloquear Cofre</Button>
          </form>
        </Card>
      </div>
    );
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <ShieldCheck className="text-green-600" /> Cofre de Senhas
          </h2>
          <p className="text-slate-500 text-sm mt-1">Gerencie as credenciais do sistema.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="secondary" onClick={() => setIsUnlocked(false)}>Bloquear</Button>
          <Button onClick={() => setIsAdding(true)}><Plus size={18} className="mr-2" /> Nova Credencial</Button>
        </div>
      </div>
      ...
      <Card>
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-500 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 font-medium">Sistema / Site</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {credentials.map((cred) => (
              <tr key={cred.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4 font-medium text-slate-800">{cred.site}</td>
                <td className="px-6 py-4 text-slate-600 font-mono text-xs">{cred.user}</td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs bg-slate-100 px-2 py-1 rounded text-slate-600">
                      {showPassword === cred.id ? cred.pass : '••••••••••••'}
                    </span>
                    <button 
                ...
                <td className="px-6 py-4 text-right space-x-2">
                  <button onClick={() => navigator.clipboard.writeText(cred.pass)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Copiar Senha">
                    <Copy size={16} />
                  </button>
                  <button onClick={() => handleDelete(cred.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Excluir">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
      ...
      <Modal isOpen={isAdding} onClose={() => setIsAdding(false)} title="Nova Credencial">
        <form onSubmit={handleAddCred} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700">Sistema / Site</label>
            <input required type="text" value={newCred.site} onChange={e => setNewCred({...newCred, site: e.target.value})} className="w-full border border-slate-300 rounded-md p-2 mt-1"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Usuário</label>
            <input required type="text" value={newCred.user} onChange={e => setNewCred({...newCred, user: e.target.value})} className="w-full border border-slate-300 rounded-md p-2 mt-1"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700">Senha</label>
            <input required type="text" value={newCred.pass} onChange={e => setNewCred({...newCred, pass: e.target.value})} className="w-full border border-slate-300 rounded-md p-2 mt-1"/>
          </div>
