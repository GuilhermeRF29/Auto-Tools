    ...
    def mostrar_dashboard(id_logado, nome_usuario):
        ...
        def criar_card_robo(titulo, icone, descricao, cor_icone, func_robo, config_datas):
            ...
            for config in config_datas:
                ...
                c_ini = ft.TextField(label=f"Ini {label}", width=135, height=45, text_size=12, read_only=True, prefix_icon=ft.Icons.CALENDAR_MONTH, focused_border_color=cor_icone, value=p_ini)
                c_fim = ft.TextField(label=f"Fim {label}", width=135, height=45, text_size=12, read_only=True, prefix_icon=ft.Icons.CALENDAR_MONTH, focused_border_color=cor_icone, value=p_fim)
            ...
            pill = ft.Container(bgcolor=ft.Colors.WHITE, border_radius=25, width=122, height=34, left=4, top=4, animate_position=ft.Animation(400, ft.AnimationCurve.DECELERATE))
            
            # Aqui eu botei o código da cor "mágica" para que ela ignore as substituições automáticas de cores primárias
            txt_padrao_btn = ft.Text("Data Padrão", size=11, weight=ft.FontWeight.BOLD, color="#000001")
            ...
            def set_modo(modo):
                ...
                txt_padrao_btn.color = "#000001" if modo == "padrao" else inactive_c
                txt_custom_btn.color = "#000001" if modo == "custom" else inactive_c
                display_padrao.visible = not is_custom
                coluna_datas_custom.visible = is_custom
                card.update()

            segmented_control = ft.Container(
                bgcolor=ft.Colors.with_opacity(0.1, ft.Colors.WHITE), border_radius=30, width=260, height=42,
                content=ft.Stack([
                    pill,
                    ft.Row([
                        ft.Container(content=txt_padrao_btn, expand=True, alignment=ft.Alignment.CENTER, on_click=lambda _: set_modo("padrao"), border_radius=30),
                        ft.Container(content=txt_custom_btn, expand=True, alignment=ft.Alignment.CENTER, on_click=lambda _: set_modo("custom"), border_radius=30),
                    ], spacing=0)
                ])
            )
            ...
            for c in config_datas:
                ...
                badges_preview.controls.append(
                    ft.Container(
                        ...
                        bgcolor=ft.Colors.with_opacity(0.12, cor_icone), padding=ft.Padding(15, 3, 15, 3), border_radius=10, border=ft.Border.all(1, ft.Colors.with_opacity(0.2, cor_icone))
                    )
                )
            ...
            area_config_inputs = ft.Column([segmented_control, display_padrao, coluna_datas_custom], spacing=15, horizontal_alignment=ft.CrossAxisAlignment.CENTER)

            badges_datas = ft.Column(spacing=8, horizontal_alignment=ft.CrossAxisAlignment.CENTER, visible=False)
                ...
                for el in elementos_data:
                    ...
                    badges_datas.controls.append(
                        ft.Container(
                            ...
                            bgcolor=ft.Colors.with_opacity(0.1, cor_icone), padding=ft.Padding(15, 5, 15, 5), border_radius=12, border=ft.Border.all(1, ft.Colors.with_opacity(0.3, cor_icone))
                        )
                    )
            ...
            btn_infinite = ft.Container(
                ...
                bgcolor=cor_icone, width=280, height=50, border_radius=18,
                ...
                animate=ft.Animation(300, "decelerate"), clip_behavior=ft.ClipBehavior.HARD_EDGE
            )
            ...
            conteudo_extra = ft.Container(
                content=ft.Column([
                    ...
                    ft.Column([ft.Container(content=texto_progresso, alignment=ft.Alignment.CENTER), btn_infinite], spacing=5, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
            ...
            # 🟢 O SEGREDO DOS FANTASMAS: Dizendo pro Flet quem são as outras abas/visões
        ...
        card_sr = criar_card_robo("Relatório Rio x SP", ft.Icons.EMAIL_ROUNDED, "E-mail e Base Rio.", ft.Colors.ORANGE_300, executar_sr, [{'label': 'Email', 'ini': SR_FIM, 'fim': SR_FIM}, {'label': 'Base', 'ini': SR_INI, 'fim': SR_INI}])
